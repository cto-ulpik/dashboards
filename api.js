...
function validateMeta({ title, subtitle, tags }) {
  const errors = [];
...
router.get('/dashboards', (req, res) => {
  ...
  res.json({
    dashboards: listDashboards({ q, tag }),
    tags: getAllTags(),
  });
});
...
router.post('/dashboards', upload.single('file'), (req, res) => {
  try {
    const { title, subtitle, tags } = req.body;
    const errors = validateMeta({ title, subtitle, tags });
...
router.put('/dashboards/:id', upload.single('file'), (req, res) => {
  try {
    ...
    updateDashboard(req.params.id, { title, subtitle, tags });
